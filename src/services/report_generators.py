...
# ─────────────────────────────────────────────────────────────────────────────
# 12. PAYSLIP DOCUMENT (single payslip → standalone PDF for the employee)
# ─────────────────────────────────────────────────────────────────────────────

class PayslipDocument:
    """Renders one Payslip — imported or business-generated — as a clean,
    standalone PDF suitable for handing to the employee: employer/employee
    header, period, taxable/non-taxable earnings, deductions, and the net
    pay total. Works for both personal-import and business-generate flows
    since both use the same Payslip/PayslipLineItem records."""

    def generate(self, ctx, payslip, output_path: str) -> str:
        code = payslip.currency_code or ctx.settings.get("base_currency", "USD")
        sym = _sym(ctx, code)
        period = ""
        ...
        rb = ReportBuilder(
            f"Payslip — {payslip.employee_name or 'Employee'}",
            (payslip.company or "") + (f"  •  {period}" if period else ""),
            ctx, output_path,
        )
        cover_lines = [
            f"Employee: {payslip.employee_name}" +
            (f" (Code {payslip.employee_code})" if payslip.employee_code else ""),
        ]
        if period:
            cover_lines.append(f"Pay Period: {period}")
        cover_lines += [
            f"Gross Pay: {sym}{payslip.gross_pay:,.2f} {code}",
            f"Total Deductions: {sym}{payslip.total_deductions:,.2f} {code}",
            f"NET PAY: {sym}{payslip.net_pay:,.2f} {code}",
        ]
        rb.add_cover(cover_lines)

        rb.section("Summary")
        rb.stat_row([
            ("Gross Pay",   f"{sym}{payslip.gross_pay:,.2f}",        code, C_GREEN),
            ("Deductions",  f"{sym}{payslip.total_deductions:,.2f}", code, C_RED),
            ("Net Pay",     f"{sym}{payslip.net_pay:,.2f}",          code, C_GOLD),
        ])
        ...
        for section, title in SECTION_TITLES_PAYSLIP.items():
            ...
            if not items:
                continue
            rb.section(title)
            rows = [[it.label, _money_cell(it.amount, sym)] for it in items]
            total = sum(it.amount for it in items)
            if len(items) > 1:
                rows.append(["TOTAL", _money_cell(total, sym)])
            rb.data_table([title, f"Amount ({code})"], rows, col_widths=[65, 25])

        rb.divider()
        rb.paragraph(
            f"Generated by WealthMap on {datetime.now().strftime('%d %B %Y at %H:%M')}",
            "BodyMuted",
        )
        return rb.save()
