...
class Transaction(Base):
    ...
    department_id         = Column(Integer, ForeignKey("departments.id"), nullable=True)
    # Set when this transaction was generated by a payslip import (one row
    # per taxable/non-taxable earning or deduction line) — lets a Payslip
    # list every transaction it produced, and a transaction show which
    # payslip (if any) it came from.
    payslip_id             = Column(Integer, ForeignKey("payslips.id"), nullable=True)
    # Set when this transaction was created by paying a Bill ("Mark as
    # Paid" in the Bills tab) — gives each bill its payment history.
    ...
    payslip        = relationship("Payslip", back_populates="transactions")
...
class PersonalLoan(Base):
    ...
    # Set only when this loan was auto-created *for* a payslip import (a
    # loan-linked deduction with no existing loan to link to). Lets
    # deleting that payslip remove the loan entirely, since it only
    # exists because of that payslip. Loans linked to an *existing* loan
    # during import are never tagged here — deleting the payslip then
    # just reverses the specific repayment instead (see LoanRepayment.payslip_id).
    payslip_id      = Column(Integer, ForeignKey("payslips.id"), nullable=True)
...
class LoanRepayment(Base):
    ...
    notes        = Column(Text, default="")
    created_at   = Column(DateTime, default=lambda: datetime.now(timezone.utc))
    # Set when this repayment was recorded automatically from a payslip's
    # loan-linked deduction — lets deleting that payslip reverse exactly
    # this repayment (subtract it back off the loan, un-settle if needed)
    # without touching any other repayment on the same loan.
    payslip_id   = Column(Integer, ForeignKey("payslips.id"), nullable=True)
...
class Payslip(Base):
    ...
    Year-To-Date summary figure exactly as it appeared on the document.
    Each taxable/non-taxable earning and each deduction is imported as its
    own sequentially-created Transaction (see PayslipLineItem.transaction
    / Transaction.payslip_id) rather than being rolled into one lump sum,
    so every individual figure is independently visible, editable, and
    reportable (e.g. in the Taxes tab) just like any other transaction —
    while still being traceable back to the payslip it came from.
    """
    ...
    transactions = relationship("Transaction", back_populates="payslip")
    attachments  = relationship("Attachment", back_populates="payslip",
                                cascade="all, delete-orphan")
...
class PayslipLineItem(Base):
    ...
    Earning/deduction rows carry a `transaction_id` pointing at the
    individual Transaction created for that specific line; loan-balance and
    YTD-summary rows are informational only and have none."""
...
class Bill(Base):
    """
    A recurring (or one-time) bill: rent, electricity, subscriptions, etc.
    Tracks how much is due, when, and how often. "Mark as Paid" creates a
    normal expense Transaction (tagged back here via Transaction.bill_id,
    so each bill keeps its full payment history) and advances `next_due`
    by the bill's frequency.
    """
    __tablename__ = "bills"

    FREQUENCIES = ("weekly", "monthly", "quarterly", "yearly", "one_time")

    id           = Column(Integer, primary_key=True)
    name         = Column(String(200), nullable=False)
    payee        = Column(String(200), default="")           # who gets paid
    category     = Column(String(100), default="Bills & Utilities")
    amount       = Column(Float, default=0.0)
    currency_id  = Column(Integer, ForeignKey("currencies.id"), nullable=True)
    account_id   = Column(Integer, ForeignKey("accounts.id"), nullable=True)  # default pay-from
    frequency    = Column(String(20), default="monthly")     # one of FREQUENCIES
    next_due     = Column(DateTime, nullable=True)
    last_paid_on = Column(DateTime, nullable=True)
    notes        = Column(Text, default="")
    is_active    = Column(Boolean, default=True)
    created_at   = Column(DateTime, default=lambda: datetime.now(timezone.utc))
